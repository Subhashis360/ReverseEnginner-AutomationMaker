...
## Device fingerprint / anti-fraud identity (do this whenever a flow is device-bound)
Many register/login/claim flows carry device-identity fields (e.g. `device`, `identification`, `deviceId`, `udid`, `imei`, `androidId`, `gaid/idfa`, AppsFlyer `af_id`, or vendor anti-fraud blobs — ShuMei/数美 `su_device_id`, NetEase YiDun/易盾 `yd_*`, Tencent/阿里 deviceToken, fingerprintjs `visitorId`). The server often counts/limits or risk-scores per device (symptoms: "limit reached on this device", "device banned", silent dedupe, risk rejections). When such fields exist you MUST:
1. **Reverse how each is generated**, not just copy the captured value. Trace the builder (`grep` the field name → its setter) and classify each id:
   - *Locally derived* (UUID from androidId/Build info, `md5(...)`, random-then-persisted) → **reproduce the exact algorithm in the generator**.
   - *Native/SDK-issued opaque blob* (ShuMei/YiDun/whitebox, or fetched from a `deviceprofile`/SDK-init endpoint) → reproduce its **format** (decoded byte-length, base64 vs url-safe/hex, prefix), and note whether the target server validates it remotely or just stores it (decide from the error behavior). If it's remotely validated against the vendor, the honest path is Frida-RPC the SDK or replay the SDK-init call; flag it if you can't.
2. **Generate a fresh, internally-consistent identity per run** and thread the SAME identity through every step of the flow (send-otp → verify → register), so any server-side binding (tokens, OTP sessions) stays coherent. Keep a `--keep-device` escape hatch to reuse a captured profile, and verify a fresh-device request is still accepted (valid sign + decrypts) before claiming it works.
3. **Ask first, unless already told.** Before adding device-id generation, ask the user whether to generate fresh device identities (one quick AskUserQuestion). **Skip the question and just implement it** if the user's prompt already requested it (e.g. "generate fresh device ids", "make it look like a new device", "bypass the per-device limit"). Stay within the operator-own-account scope in `references/automation.md` — this is for testing your own flow on a fresh install, not mass identity manufacturing.
